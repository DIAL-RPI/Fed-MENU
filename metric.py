...
def eval(pd_path, gt_entries, label_map, cls_num, metric_fn, calc_asd=True, keep_largest=False):
    ...
    if not os.path.exists(df_fn):
        ...
        for [d_name, casename, gt_fname] in gt_entries:
            ...
            for c in tgt_labels:
                ...
                dsc = cal_dsc(pd, gt)
                if calc_asd and np.sum(pd) > 0 and np.sum(gt) > 0:
    ...
    for c in range(cls_num):
        ...
        print(print_line)
        result_lines += '{}\n'.format(print_line)
    dsc_mean = dsc_mean / dsc_mean_num
    asd_mean = asd_mean / asd_mean_num
    hd_mean = hd_mean / hd_mean_num
    dsc = np.array(dsc)
    asd = np.array(asd)
    hd = np.array(hd)
